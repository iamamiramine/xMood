...
class PEmoPipeline(LightningDataModule):
    def __init__(self, config, fix_config) -> None:
        ...
        self.labels_path = getattr(self.fix_config.task, 'labels_path', "../dataset/split")

    def get_fl(self):
        # Get labels path from config or use default
        labels_path = self.labels_path
        
        if self.split == "TRAIN":
            self.fl = pd.read_csv(os.path.join(labels_path, "train.csv"), index_col=0)
        elif self.split == "VALID":
            self.fl = pd.read_csv(os.path.join(labels_path, "val.csv"), index_col=0)
        elif self.split == "TEST":
            self.fl = pd.read_csv(os.path.join(labels_path, "test.csv"), index_col=0)
        else:
            print("Split should be one of [TRAIN, VALID, TEST]")

    def setup(self, stage: Optional[str] = None):
        # Get CSV path for mood labels if specified in config
        labels_path = self.labels_path
    ...
    @classmethod
    def get_dataset(cls, dataset_builder: Callable, feature_path, labels, split, cls_type, pad_idx, csv_path=None) -> Dataset:
        dataset = dataset_builder(feature_path, labels, split, cls_type, pad_idx, csv_path)
        return dataset
