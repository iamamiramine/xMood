...
from domain.constants.harmony_constants import (
    get_chord_maps,
            ...
            if 3 in sequence:
                ...
                if 8 in sequence:
                    ...
                    if 10 in sequence:
                        ...
                        quality = "maj"
            # decide score
            maps = get_chord_maps().get(quality)
            _notes = [n for n in sequence if n not in maps]
            score = 0
            for n in _notes:
                if n in get_chord_outsiders_1().get(quality):
                    score -= 1
                elif n in get_chord_outsiders_2().get(quality):
                    score -= 2
                elif n in get_chord_insiders().get(quality):
                    score += 10
...
def find_chord(chroma, threshold=10):
    ...
    if np.sum(chroma) == 0:
        ...
        return (
            get_pitch_classes()[root_note],
            quality,
            get_pitch_classes()[bass_note],
            score,
        )


def greedy(candidates, max_tick, min_length):
    chords = []
    # start from 0
    start_tick = 0
...
def dynamic_chords(candidates, max_tick, min_length):
...
def get_candidate_chords(chroma, max_tick, intervals=[1, 2, 3, 4]):
    candidates = {}
    for interval in intervals:
        for start_beat in range(max_tick):
            ...
            if start_beat not in candidates:
                ...
                candidates[start_beat][end_beat] = (
                    root_note,
                    quality,
                    bass_note,
                    score,
