...
from application.classifier.services.classifier_service import train as classifier_train
...
from application.psuedo_labeller.services.pseudo_labeller_service import (
    create_mood_labels,
    classify_image_mood,
...
class PipelineJobService:
    ...
    def _build_service_registry(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "classifier": {
                ...
                "extract_features": classifier_train,  # Same function with different params
            },
            "encoder": {
            ...
            "pseudo_labeller": {
                "create_mood_labels": create_mood_labels,
                "classify_image": classify_image_mood,
                "batch_classify": batch_classify_images,
            }
        }
    ...
    def _build_parameter_registry(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "classifier": {
                ...
                "extract_features": MidiFeatureExtractParameters,
                "predict_mood": PredictMoodParameters,
                "batch_predict": BatchPredictParameters,
            },
            "encoder": {
    ...
    def parse_service_config_to_parameters(
        ...
        try:
            ...
            if service_name not in self.parameter_registry:
                raise ConfigurationError(f"Service '{service_name}' not found in parameter registry")
            ...
            raise ConfigurationError(f"Configuration parsing failed: {str(e)}")
    ...
    def run_pipeline(
        ...
        try:
            ...
            raise ConfigurationError(f"Pipeline execution failed: {str(e)}")
    ...
    def _get_default_pipeline_definition(self, config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        service_order = [
            ...
            ("pseudo_labeller", "create_mood_labels"),
    ...
    def get_pipeline_status(self, pipeline_job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigurationError(f"Pipeline status retrieval failed: {str(e)}")
    ...
    def cancel_pipeline(self, pipeline_job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigurationError(f"Pipeline cancellation failed: {str(e)}")
