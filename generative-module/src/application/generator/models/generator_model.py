...
class MIDIGeneratorModule(pl.LightningModule):
    def __init__(
        ...
        # self.moods_in = nn.Embedding(len(self.moods_vocab), self.d_model)
        # self.pos_neg_in = nn.Embedding(len(self.moods_vocab), self.d_model)
        self.emotion_in = nn.Embedding(len(self.emotion_vocab), self.d_model)

        # Update projection layer to handle concatenated embeddings (3 * d_model for symb + latent + moods)
        # self.symb_proj = nn.Linear(3 * self.d_model, self.d_model, bias=False)
    ...
    def encode(self, z=None, symb_bar_ids=None):
        ...
        if not bar_level_embeddings:
            ...
            if len(padded_bar_embeddings) == 2:
                symb_emb, latent_emb = padded_bar_embeddings
            elif len(padded_bar_embeddings) == 1:
                if symb_emb is not None:
                    symb_emb = padded_bar_embeddings[0]
                else:
                    latent_emb = padded_bar_embeddings[0]

            # Collect non-None embeddings
            z_embeddings = [emb for emb in [symb_emb, latent_emb] if emb is not None]

            # Handle bar-level features as in original code
            if len(z_embeddings) == 1:
                z_emb = z_embeddings[0]  # Use single embedding directly
    ...
    def _build_z_input(self, batch):
        ...
        symb_bar_ids = batch.get("symb_bar_ids", None)
    ...
    def sample(self, batch=None, max_length=256, max_bars=-1, temp=0.8, pad_token=PAD_TOKEN, eos_token=EOS_TOKEN, verbose=0):
        ...
        for i in range(curr_len - 1, max_length):
            ...
            if z is not None:
                if z.get("bar_symbolic") is not None:
                    symb = z["bar_symbolic"]
                    ...
                    if bars_changed:
                        ...
                        for j in range(batch_size):
                            ...
                            offset = min(self.context_size, symb.size(1) - idx)

                            z_[j, :offset] = symb[j, idx : idx + offset]
