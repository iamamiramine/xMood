...
from application.symbolic.helpers.vocab_helper import get_bars, mask_bar_tokens, get_bos_eos_events
from domain.constants.paths_constants import LATENTS_PATH
...
from application.latent.models.vq_ema_model import VectorQuantizeEMA
from core.symbolic.models.vocab_model import RemiVocab
from core.dataloader.helpers.dataloader_helper import save_latents_hdf5
...
class VqVaeModule(LightningModule):
    def __init__(
        ...
        self.d_model = d_model  #  size of the embeddings and the hidden layers throughout the symbolic and decoder
    ...
    def encode(self, x):
        ...
        if self.disable_vq:
            ...
            dist = self.trainer.accelerator if self.training else None
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        output_path = os.path.join(LATENTS_PATH, self.dataset_name)

        for i, event_ids in enumerate(events):
            file = batch["files"][i]
            try:
                # Check if latents file already exists
                latents_filename = f"{os.path.basename(file)}_latents.h5"
                latents_filepath = os.path.join(output_path, latents_filename)
                if os.path.exists(latents_filepath):
                    print(f"Skipping {os.path.basename(file)}: latents already exist")
                    continue
                ...
                # Save latents and codes to HDF5
                save_latents_hdf5(output_path, file, latents, codes)
