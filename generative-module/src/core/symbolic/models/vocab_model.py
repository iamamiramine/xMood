...
# Constants - Harmony
from domain.constants.harmony_constants import get_all_major_minor_keys, get_pitch_classes, get_chord_qualities
...
from domain.constants.token_constants import (
    ...
    # XMIDI Emotion constants
    EMOTION_ANGRY_KEY,
    EMOTION_EXCITING_KEY,
    EMOTION_FEAR_KEY,
    EMOTION_FUNNY_KEY,
    EMOTION_HAPPY_KEY,
    EMOTION_LAZY_KEY,
    EMOTION_MAGNIFICENT_KEY,
    EMOTION_QUIET_KEY,
    EMOTION_ROMANTIC_KEY,
    EMOTION_SAD_KEY,
    EMOTION_WARM_KEY,
)
...
class SymbolicFeaturesVocabPos(Vocab):
    """
    A specialized vocabulary class for musical piece symbolic features and statistical features.
...
class MoodsVocab(Vocab):
    """
    A specialized vocabulary class for representing musical moods and their temporal progression.

    This class creates a vocabulary that combines:
    1. Structural Elements:
       - Bar markers for temporal organization of moods

    2. moods Categories (0.0000-1.0000 intensity for each):
       - All standard moods from the moods classification model
       Each moods intensity is represented with 4 decimal precision

    Each mood is represented with 4 decimal precision (0.0000-1.0000),
    allowing for precise moods intensity representation at each bar.
    ...
    def __init__(self):
        """
        Initialize the moods vocabulary with structural and moods intensity tokens.

        The initialization process:
        1. Creates bar position tokens for temporal organization
        2. Creates tokens for each moods category with 4 decimal precision
        3. Combines tokens in a specific order for consistent indexing

        Token formats:
        - Bar tokens: "Bar_0", "Bar_1", etc.
        - moods tokens: "{moods}_{0.0000}" to "{moods}_{1.0000}" for each moods

        Note:
            The decimal range (0.0000-1.0000) allows for fine-grained moods intensity
            representation, where 0.0000 represents absence and 1.0000 represents maximum
            intensity of that moods.
        """
        # Generate structural tokens
        bar_tokens = [f"Bar_{i}" for i in range(MAX_N_BARS)]

        # Function to generate moods tokens with 4 decimal precision
        def generate_moods_tokens(moods_key):
            return [f"{moods_key}_{i/10000:.4f}" for i in range(10001)]

        # Generate moods intensity tokens for all moods
        moods_tokens = []
        for moods_key in [
            HAPPY_KEY,
            DRAMATIC_KEY,
            ...
            moods_tokens.extend(generate_moods_tokens(moods_key))

        # Combine all tokens in a specific order
        self.tokens = bar_tokens + moods_tokens
...
class EmotionsNewVocab(Vocab):
    """
    A specialized vocabulary class for representing emotions from the XMIDI dataset.
    ...
    2. Emotion Categories:
       - All emotion types found in the XMIDI dataset
       Each emotion is represented as a simple categorical token.
    ...
    def __init__(self):
        """
        Initialize the emotions vocabulary with structural and emotion tokens.

        The initialization process:
        1. Creates bar position tokens for temporal organization
        2. Creates tokens for each emotion category
        3. Combines tokens in a specific order for consistent indexing

        Token formats:
        - Bar tokens: "Bar_0", "Bar_1", etc.
        - Emotion tokens: "Emotion_Happy", "Emotion_Sad", etc.

        Note:
            Unlike MoodsVocab, this vocabulary uses simple categorical emotion tokens
            without intensity values, as the XMIDI dataset provides discrete emotion labels.
        """
        # Generate structural tokens
        bar_tokens = [f"{BAR_KEY}_{i}" for i in range(MAX_N_BARS)]

        # Generate emotion tokens for all emotions in XMIDI dataset
        emotion_tokens = [
            EMOTION_ANGRY_KEY,
            EMOTION_EXCITING_KEY,
            EMOTION_FEAR_KEY,
