...
class SymbolicFeaturesParameters(BaseModel):
    ...
    description_out_dir: Annotated[str, Query(description="Symbolic Features output directory")] = ""
...
class LatentRepresentationDatasetParameters(BaseModel):
    ...
    target_batch_size: Annotated[
        int,
        Query(description="Number of samples in each backward step, gradients will be accumulated over TARGET_BATCH_SIZE//BATCH_SIZE batches"),
    ] = 256
...
class VQVAEParameters(BaseModel):
    ...
    n_groups: Annotated[
        int,
        Query(description="Number of groups to split the latent vector into before discretization"),
    ] = 16
    d_latent: Annotated[int, Query(description="Dimensionality of the latent space")] = 1024

    lr: Annotated[
        float,
        Query(description="Initial learning rate, will be decayed after constant warmup of WARMUP_STEPS stepse"),
    ] = 1e-4
    lr_schedule: Annotated[str, Query(description="Learning rate schedule")] = "const"  # "sqrt_decay"
    warmup_steps: Annotated[int, Query(description="Number of learning rate warmup steps")] = 4000
    ...
    windowed_attention_pr: Annotated[float, Query(description="Windowed Attention PR")] = 0.0
    ...
    automatic_optimization: Annotated[bool, Query(description="Automatic Optimization")] = False
    beta: Annotated[float, Query(description="Beta")] = 0.02
    cycle_length: Annotated[int, Query(description="Cycle Length")] = 2000
    position_embedding_type: Annotated[str, Query(description="Position Embedding Type")] = "relative_key_query"
...
class TSNEParameters(BaseModel):
    ...
    n_iter_without_progress: Annotated[int, Query(description="Number of Iterations Without Progress")] = 300
...
class CorrelationParameters(BaseModel):
    dataset_name: Annotated[str, Query(description="Name of Dataset")] = ""
    correlation_type: Annotated[str, Query(description="Correlation Type")] = "spearmann"
